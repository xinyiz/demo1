...
if __name__ == '__main__':
    ...
    N = 1000 # number of input samples(sine waves)
    number_hu = 60 
    num_mixtures = 1 
    train_test_split = 0.99 # proportion of samples to use for training/prediction   
    ...
    data = torch.load('data_generator/particle_spring_1000_4.pt')
    ...
    if(not PREDICT):
        ...
        optimizer = optim.Adam(srnn.parameters(),lr=0.00005)
        ...
        for i in range(EPOCHS):
            ...
            for input_b,target_b in zip(in_batches,target_batches):
                ...
                optimizer.step()
        ### SAVE MODEL ###
        torch.save(srnn.state_dict(), "spring_model_lstm_1.pth")
        np.save("spring_model_train_loss", train_loss)
        plot_loss(train_loss)

        srnn.load_state_dict(torch.load("spring_model_lstm_1.pth"))
        i = EPOCHS
        future = L 
        end_idx = int(L/2)
        ...
        for j in range(1,2):
            ...
            draw(y,j, 'm')
        plt.savefig('spring_lstm_predict%d.pdf'%i)
        plt.close()
